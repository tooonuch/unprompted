...
In my testing, I have found that `[img2pez]` is often more effective than Textual Inversion for reproducing concepts, while both are less effective than LoRA or finetuning.
...
Supports the amusingly-named `prompt_bs` argument, which is described by the technique's authors as "number of initializations." Defaults to 1.
