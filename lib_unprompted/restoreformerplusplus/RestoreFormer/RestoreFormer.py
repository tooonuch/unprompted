...
from .modules.vqvae.vqvae_arch import VQVAEGANMultiHeadTransformer
...
class RestoreFormer():
	...
	def __init__(self, model_path, upscale=2, arch='RestoreFromerPlusPlus', bg_upsampler=None, device=None):
		...
		self.face_helper = FaceRestoreHelper(upscale, face_size=512, crop_ratio=(1, 1), det_model='retinaface_resnet50', save_ext='png', use_parse=True, device=self.device, model_rootpath=None)

		if model_path.startswith('https://'):
			model_path = load_file_from_url(url=model_path, model_dir=os.path.join(ROOT_DIR, 'experiments/weights'), progress=True, file_name=None)
