...
import lib_unprompted.tris.CLIP.clip as clip

from lib_unprompted.tris.model.attn import PixelAttention
...
class TRIS(nn.Module):
	def __init__(self, bert_tokenizer, backbone, max_query_len):
		super().__init__()
		...
		##################
		self.final_seg1 = nn.Sequential(ConvBNRelu(32, 32, kernel_size=3, stride=1, padding=1), nn.Conv2d(32, 1, kernel_size=1, bias=False))
		self.final_seg2 = nn.Sequential(ConvBNRelu(64, 32, kernel_size=3, stride=1, padding=1), nn.Conv2d(32, 1, kernel_size=1, bias=False))
		self.final_seg3 = nn.Sequential(ConvBNRelu(128, 64, kernel_size=3, stride=1, padding=1), nn.Conv2d(64, 1, kernel_size=1, bias=False))
		self.final_seg4 = nn.Sequential(ConvBNRelu(256, 64, kernel_size=3, stride=1, padding=1), nn.Conv2d(64, 1, kernel_size=1, bias=False))
		# ##################
